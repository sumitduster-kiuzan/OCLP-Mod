...
class SysPatchStartFrame(wx.Frame):
    """
    Create a frame for Installing Root Patches
    ...
    def __init__(self, parent: wx.Frame, title: str, global_constants: constants.Constants, screen_location: tuple = None, patches: dict = {}):
        logging.info("Initializing Install Root Patches window")
    ...
    def _kdk_download(self, frame: wx.Frame = None) -> bool:
        ...
        header = wx.StaticText(frame, label="Downloading KDK", pos=(-1,5))
        header.SetFont(gui_support.font_factory(19, wx.FONTWEIGHT_BOLD))
        header.Centre(wx.HORIZONTAL)

        subheader = wx.StaticText(frame, label="Fetching KDK catalog...", pos=(-1, header.GetPosition()[1] + header.GetSize()[1] + 5))
        ...
        if self.kdk_obj.validate_kdk_checksum() is False:
            ...
            msg = wx.MessageDialog(frame, f"KDK MD5 mismatch: {self.kdk_obj.error_msg}", "Error", wx.OK | wx.ICON_ERROR)
            msg.ShowModal()
            return False
    ...
    def _metallib_download(self, frame: wx.Frame = None) -> bool:
        ...
        subheader = wx.StaticText(frame, label="Fetching MetallibSupportPkg catalog...", pos=(-1, header.GetPosition()[1] + header.GetSize()[1] + 5))
        subheader.SetFont(gui_support.font_factory(13, wx.FONTWEIGHT_NORMAL))
        subheader.Centre(wx.HORIZONTAL)
        ...
        subheader.SetLabel("Installing Metallib...")
        subheader.Centre(wx.HORIZONTAL)
    ...
    def _generate_modal(self, patches: dict = {}, variant: str = "Installing Root Patches"):
        """
        Create UI for installing/uninstalling root patches
        """
        # Support both legacy Chinese labels and new English labels
        supported_variants = [
            "正在安装驱动补丁",
            "正在卸载驱动补丁",
            "Installing Root Patches",
            "Uninstalling Root Patches",
        ]
        ...
        if variant in ["正在安装驱动补丁", "Installing Root Patches"]:
            # Label
            ...
            if i == 20:
                ...
                patch_label = wx.StaticText(dialog, label="No patches could be applied", pos=(label.GetPosition()[0], label.GetPosition()[1] + 20))
                patch_label.SetFont(gui_support.font_factory(13, wx.FONTWEIGHT_BOLD))
                patch_label.Centre(wx.HORIZONTAL)
        else:
            patch_label = wx.StaticText(dialog, label="Revert to most recent snapshot", pos=(-1, title.GetPosition()[1] + 30))
        ...
        return_button = wx.Button(dialog, label="Return", pos=(10, text_box.GetPosition()[1] + text_box.GetSize()[1] + 5), size=(150, 30))
    ...
    def start_root_patching(self):
        logging.info("Starting Install Root Patches")
        ...
        self._generate_modal(self.patches, "Installing Root Patches")
    ...
    def revert_root_patching(self):
        ...
        self._generate_modal(self.patches, "Uninstalling Root Patches")
        self.return_button.Disable()
    ...
    def _post_patch(self):
        ...
        if self.constants.needs_to_open_preferences is False:
            gui_support.RestartHost(self.frame_modal).restart(message="Root patching succeeded!\n\nReboot now?")
            return

        if self.constants.detected_os >= os_data.os_data.ventura:
            gui_support.RestartHost(self.frame_modal).restart(message="Root patching succeeded!\nIf macOS prompts to open System Settings to approve new kexts, you can ignore this. Your system will be ready after reboot.\n\nReboot now?")
            return

        # Create dialog box to open System Preferences -> Security and Privacy
        self.popup = wx.MessageDialog(
            self.frame_modal,
            "We just finished applying patches to the root volume!\n\nHowever, Apple requires users to manually approve kernel extensions before they can be used on next reboot.\n\nOpen System Preferences now?",
            "Open System Preferences?",
            wx.YES_NO | wx.ICON_INFORMATION
        )
        self.popup.SetYesNoLabels("Open System Settings", "Ignore")
