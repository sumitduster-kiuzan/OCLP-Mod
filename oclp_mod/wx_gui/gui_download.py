    ...
    def _generate_elements(self, frame: wx.Dialog = None) -> None:
        ...
        while self.download_obj.is_active():
            ...
            if percentage == -1:
                ...
                amount_str = f"{utilities.seconds_to_readable_time(self.download_obj.get_time_remaining())} remaining - {utilities.human_fmt(self.download_obj.downloaded_file_size)} of {utilities.human_fmt(self.download_obj.total_file_size)} at {utilities.human_fmt(self.download_obj.get_speed())}/s"
