...
class MainFrame(wx.Frame):
    ...
    def _generate_elements(self) -> None:
        ...
        # Title label: Open Legacy Mode v{X.Y.Z}
        title_label = wx.StaticText(self, label=f"Open Legacy Mode by laobamac {'' if self.constants.special_build else ''}{self.constants.patcher_version}{'' if not self.constants.commit_info[0].startswith('refs/tags') else ''}", pos=(-1, 10))
        ...
        menu_buttons = {
            ...
            "Post-Install Root Patch": {
                ...
                "description": [
                    "Install hardware root patches",
                    "(apply after installing a",
                    "newer macOS)",
                ],
            ...
            "Get Help": {
                ...
                "description": [
                    "Open Legacy Mode resources",
                    "Localized by laobamac",
                ],
        ...
        for button_name, button_function in menu_buttons.items():
            ...
            if button_name == "Build and Install OpenCore":
                ...
                if self.constants.detected_os < os_data.os_data.big_sur:
                  ...
                  ml_button = wx.Button(self, label="Download MetalLib", pos=(button_x + 190, button.GetPosition()[1]), size=(100, 30))
    ...
    def _preflight_checks(self):
        if (
            ...
            pop_up = wx.MessageDialog(
                self,
                f"We detected you're booting an OpenCore build for a different machine: {self.constants.computer.build_model}\n\nConfigurations must match a specific unit and cannot be reused across different Macs.\n\nPlease build and install a new OpenCore configuration, then reboot your Mac.",
        ...
        if "--update_installed" in sys.argv and self.constants.has_checked_updates is False and gui_support.CheckProperties(self.constants).host_can_build():
            ...
            pop_up = wx.MessageDialog(
                self,
                f"Open Legacy Mode has been updated to the latest version: {self.constants.patcher_version}\n\nWould you like to update OpenCore and your root volume patches?",
                "Update complete!",
...
请前往Github RELEASE页面查看"""
        ...
        frame = wx.Dialog(None, -1, title="Open Legacy Mode update available", size=(650, 500))
        frame.SetMinSize((650, 500))
        frame.SetWindowStyle(wx.STAY_ON_TOP)
        panel = wx.Panel(frame)
        sizer = wx.BoxSizer(wx.VERTICAL)
        sizer.AddSpacer(10)
        self.title_text = wx.StaticText(panel, label="A new Open Legacy Mode build is available!")
        self.description = wx.StaticText(panel, label=f"Open Legacy Mode {oclp_version} released. You have {self.constants.patcher_version}{'' if not self.constants.commit_info[0].startswith('refs/tags') else ''}. Update now?")
